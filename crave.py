...
#response = requests.post(url=url, data=json.dumps(crave_tools.search_body(query)))
...
def search_shows(query: str, quiet: bool = False) -> None:
    ...
    url, body = crave_tools.search_body(query)
...
def list_episodes(show_path: str, quiet: bool = False) -> dict[str, str]:
    ...
    req_url, headers = crave_tools.show_path_url(show_path)
    ...
    req_url, headers = crave_tools.show_id_url(show_id, show_path)
    ...
    if show["mediaType"] == "SERIES":
        ...
        for season in show["seasons"]:
            ...
            req_url, headers, body = crave_tools.season_id_url(season["id"])
...
def get_episodes_info(episode):
    ...
    try:
        ...
        episode_info["description"] = episode["shortDesc"]
...
def get_download(url, latest, seasons_episodes, options):
    ...
    for episode in chosen_episodes["episodes"]:
        ...
        download_content(episode["axisId"], episode["destinationCode"], options)


def download_content(id: int, service_hub_name: str, options):
    episode_info_url, headers = crave_tools.episode_id_url(id, service_hub_name)
    ...
    while "errors" in episode_info_resp.keys():
        r = requests.get(episode_info_url, headers=headers)
        episode_info_resp = r.json()

    config_info_url = crave_tools.service_config("crave")
    config_resp = requests.get(config_info_url).json()

    episode_second_id_url = crave_tools.second_episode_id(id, "crave_atexace")
    ...
    mpd_req_url = crave_tools.mpd_url(id, second_id, "crave_atexace")

    low_res_mpd = "Error"
    while "Error" in low_res_mpd:
        low_res_mpd = requests.get(mpd_req_url, headers=options["headers"]).text

    mpd_url: str = low_res_mpd.replace("zbest", "zultimate")

    licence_url: str = config_resp["api"]["drmLicenceServerUrl"]
    for value in options["headers"].values():
        licence_url += "/widevine?jwt=" + value.replace("Bearer ", "")
...
def connect():
    ...
    return(bell_tokens.login(settings_path, "crave"))
...
def list(args):
    if len(args) > 2:
        if not crave_tools.validate_url(args[2]):
...
def download(args):
        ...
        url = args[2]
        if len(args) > 2:
            if not crave_tools.validate_url(url):
...
#if "search" == "search":
#    args.append("search")
#    args.append("completement lycée")
#    search(args)
